class HeapMax():
    ...
    def float(self, index):
        """Reorganiza elementos hacia arriba si son mayores"""
        # father = (index-1) // 2
        ...
        father = (index-1) // 2
        while index > 0:
            current_priority = self.elements[index][0]  # Prioridad actual
            father_priority = self.elements[father][0]  # Prioridad del padre

            if current_priority > father_priority:
                self.interchange(index, father)
    ...
    def sink(self, index):
        """Reorganiza elementos hacia abajo si son menores"""
    ...
    def heapify(self, elements):
        """Organiza una lista completa"""
        self.elements = elements
        for i in range(len(self.elements)):
            self.float(i)

    def sort(self):
        """Organiza los elementos en forma de lista"""
        result = []
    ...
    def arrive(self, value, priority):
        """Agrega elementos con su prioridad"""
        self.add([priority, value])

    def atention(self):
        """Atiende al primer elemento"""
        return self.remove()

    def atention_mas(self, value, priority=None):
        """Atiende al primer elemento y agrega otro"""
        if value:
            if priority:
    ...
    def change_priority(self, index, new_priority):
        """Cambia la prioridad de un elemento con su indice"""
...
class HeapMin():
    ...
    def float(self, index):
        """Reorganiza elementos hacia arriba si son menores"""
        father = (index-1) // 2
        while index > 0 and self.elements[index] < self.elements[father]:
            self.interchange(index, father)
            index = father
    ...
    def sink(self, index):
        """Reorganiza elementos hacia abajo si son mayores"""
        left_child = (index * 2) + 1
        control = True
        while control and left_child < len(self.elements):
            right_child = (index * 2) + 2
